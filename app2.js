...
function ChangePassForm({ session, users, onChangePass }) {
  ...
  function handleSave() {
    const oldPass = refOld.current ? refOld.current.value : "";
    const newPass = refNew.current ? refNew.current.value : "";
    const confirm = refConf.current ? refConf.current.value : "";
    setMsg(null);
    const me = (users||[]).find(function(u){ return u.user === session.user; });
    if (!me) return;
    ...
    setMsg({ type: "ok", text: "Senha alterada!" });
    if (refOld.current) refOld.current.value = "";
    if (refNew.current) refNew.current.value = "";
    if (refConf.current) refConf.current.value = "";
  }

  const inputStyle = { width: "100%", background: "#111622", border: "1px solid #1e2636", color: "#e0e8ff", borderRadius: 8, padding: "11px 44px 11px 14px", fontSize: 14, fontFamily: "monospace", boxSizing: "border-box" };
  const labelStyle = { fontSize: 10, color: "#4a5a7a", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 6 };
  const eyeBtn = { position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "transparent", border: "none", cursor: "pointer", padding: "4px" };

  return React.createElement("div", { style: { background: "#0d1117", border: "1px solid #1e2636", borderRadius: 14, padding: "18px 16px" } },
    React.createElement("div", { style: { marginBottom: 14 } },
      ...
      React.createElement("div", { style: { position: "relative" } },
        ...
        React.createElement("button", { type: "button", onMouseDown: function(e){ e.preventDefault(); setShowOld(function(v){return !v;}); }, style: eyeBtn },
          React.createElement(EyeIcon, { open: showOld })
        )
      )
    ),
    React.createElement("div", { style: { marginBottom: 14 } },
      ...
      React.createElement("div", { style: { position: "relative" } },
        ...
        React.createElement("button", { type: "button", onMouseDown: function(e){ e.preventDefault(); setShowNew(function(v){return !v;}); }, style: eyeBtn },
          React.createElement(EyeIcon, { open: showNew })
        )
      )
    ),
    React.createElement("div", { style: { marginBottom: 14 } },
      React.createElement("div", { style: labelStyle }, "Confirmar nova"),
      React.createElement("div", { style: { position: "relative" } },
    ...
    msg ? React.createElement("div", { style: { background: msg.type === "ok" ? "#00e5a018" : "#ff4d6d12", border: "1px solid " + (msg.type === "ok" ? "#00e5a044" : "#ff4d6d44"), borderRadius: 8, padding: "9px 12px", fontSize: 12, color: msg.type === "ok" ? "#00e5a0" : "#ff8099", marginBottom: 14, textAlign: "center" } }, msg.type === "ok" ? "\u2713 " : "", msg.text) : null,
    React.createElement("button", { onClick: handleSave, style: { width: "100%", background: "linear-gradient(135deg,#1a3a6a,#2a5aaa)", border: "1px solid #4a8aff44", color: "#e0f0ff", borderRadius: 10, padding: "12px 0", cursor: "pointer", fontSize: 14, fontWeight: 700, fontFamily: "monospace" } }, "Salvar nova senha")
  );
}
