...
const APP_VERSION = "v1.0.3 · Firebase save fix · app1v10/app2v10/app3v10";
const APP_DEV = "Fabr\xEDcio Ferreira";
const STORAGE_KEY = "canaleta_machines_v1";
let _machinesSaveChain = Promise.resolve();
let _machinesSaveRevision = 0;

function firebaseSaveError(scope, error) {
  window._firebaseLastError = { scope, message: error && error.message ? error.message : String(error), at: Date.now() };
  console.warn("Firebase save error [" + scope + "]:", error);
}
...
async function salvarRegistroFirebase(machineId, machineName, rec) {
  try {
    ...
    await window._fbDb.collection("turnos").doc(id).set({ machineId, machineName, ...rec, _savedAt: Date.now() }, { merge: true });
    window._firebaseLastError = null;
    return true;
  } catch (e) {
    firebaseSaveError("turnos/" + (rec.recordId || machineId), e);
    return false;
  }
}
...
async function persistSave(machines) {
  ...
  const revision = ++_machinesSaveRevision;
  _machinesSaveChain = _machinesSaveChain.catch(() => {}).then(async () => {
    // Escritas intermediárias podem ser descartadas; somente o estado mais novo vai ao Firebase.
    if (revision !== _machinesSaveRevision) return true;
    if (!window._fbReady || !window._fbDb) return false;
    try {
      await window._fbDb.collection("canaleta").doc("machines").set({ data: JSON.stringify(machines), updatedAt: Date.now() }, { merge: true });
      window._firebaseLastError = null;
      return true;
    } catch (e) {
      firebaseSaveError("canaleta/machines", e);
      return false;
    }
  });
  return _machinesSaveChain;
}
