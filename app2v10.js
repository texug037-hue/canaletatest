...
function Detail({ machine, machines, users: machines_users, session, onBack, onUpdate, onFinalize }) {
  ...
  function handleFinalize() {
    ...
    setObservation("");
    const machineSave = onUpdate(updated);
    const recordSave = salvarRegistroFirebase(m.id, m.name, rec);
    Promise.all([Promise.resolve(machineSave), recordSave]).then(([machineOk, recordOk]) => {
      if (window._fbReady && (machineOk === false || recordOk === false)) {
        window.alert("O dia foi salvo neste aparelho, mas o Firebase não confirmou uma das gravações. Verifique a conexão e tente salvar novamente.");
        return;
      }
